...
mpiexec --allow-run-as-root -n $NGPU python train_ddbm.py --exp=$EXP \
    --attention_resolutions $ATTN --class_cond False --use_scale_shift_norm True \
    --dropout 0.1 --ema_rate 0.9999 --batch_size $BS --image_size $IMG_SIZE \
    --lr 0.0001 --num_channels $NUM_CH --num_head_channels 64 --num_res_blocks $NUM_RES_BLOCKS \
    --resblock_updown True ${COND:+ --condition_mode="${COND}"} ${MICRO:+ --microbatch="${MICRO}"} \
    --pred_mode=$PRED --schedule_sampler $SAMPLER ${UNET:+ --unet_type="${UNET}"} \
    --use_fp16 $USE_16FP --attention_type $ATTN_TYPE --weight_decay 0.0 \
    --weight_schedule bridge_karras ${BETA_D:+ --beta_d="${BETA_D}"} ${BETA_MIN:+ --beta_min="${BETA_MIN}"}  \
    ...
    ${CKPT:+ --resume_checkpoint="${CKPT}"} --g_equiv=$G_EQUIV --g_output=$G_OUTPUT --g_reg=$G_REG